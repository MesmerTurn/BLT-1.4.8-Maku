# BLT 5.2.4 — port for Bannerlord 1.4.8

This is upstream **Bannerlord Twitch (BLT) 5.2.4** ported from game version **1.3.15** to
**1.4.8**. It is the plain BLT — no LOTR, TAOM or Rebellion of the Peasants content, and no
`MakeBltGreatAgain` / `PeasantRebellionPerks` add-ons.

Grab the ready-to-use build from the **[Releases](../../releases)** tab, or build from source.

## Install

Extract the four module folders (`BannerlordTwitch`, `BLTAdoptAHero`, `BLTBuffet`,
`BLTConfigure`) into your Bannerlord `Modules\` directory, then enable them in the launcher.

## What changed for 1.4.8

Only compatibility work — no gameplay changes, no new features.

| 1.3.15 API | 1.4.8 |
|---|---|
| version gate `v1.3.15` | `v1.4.8` |
| `MissionAgentSpawnLogic` | renamed to `DefaultBattleMissionAgentSpawnLogic` |
| `IAgentOriginBase` | new required member `IsInSameArmyAsPlayer` |
| `ShipAgentSpawnLogic` | gone — patch now resolves the naval spawn-logic type by name and skips cleanly via `Prepare()` if absent |
| `TradeAgreementsCampaignBehavior.HasTradeAgreement(a, b)` | added `out TradeAgreement` parameter |
| `Clan.CommanderLimit` | `Clan.WarPartyLimit` |
| `ArmyManagementCalculationModel.GetMobilePartiesToCallToArmy` | removed — replaced with a local equivalent |
| `MBEquipmentRoster.HasEquipmentFlags(EquipmentFlags...)` | replaced by the `EquipmentCategories` flags enum |
| `Mission.SpawnTroop(..., forceDismounted:)` | parameter removed |
| `SetPartyAiAction.GetActionForRaidingSettlement` | added `isTargetingPort` parameter |

### Notes on two judgement calls

- **`IsCombatantTemplate` has no 1.4.8 equivalent.** Where clan-founding gear was previously
  filtered by noble + combatant flags, it now filters on `IsLordTemplate` and excludes the
  child/teenager templates instead, which is what that filter was really keeping out.
- **`isTargetingPort`** is passed the same value as `isFromPort`, mirroring the adjacent
  `GetActionForVisitingSettlement` call.

The `ShipAgentSpawnLogic` change matters beyond naval play: a `[HarmonyPatch(typeof(...))]`
against a type that no longer exists throws out of `PatchAll` and takes **every** patch in the
assembly with it, leaving the whole mod dead with no obvious cause.

## Licence & credits

LGPL 2.1, same as upstream. Original mod by the
[Bannerlord-Twitch](https://github.com/jazz-ttv/Bannerlord-Twitch) authors — all credit for BLT
itself goes to them; this repository only carries the 1.4.8 compatibility work.
